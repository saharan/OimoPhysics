package com.element.oimo.physics.collision.narrow {
	...
	public class BoxCylinderCollisionDetector extends CollisionDetector {
				...
				while (true) { // refinement phase
					...
					if (nx * v1x + ny * v1y + nz * v1z >= 0 && !hit) { // hit
						...
						while (true) {
							...
							var apz:Number = pz - az;
							
							var d1:Number = abx * apx + aby * apy + abz * apz;
							var d2:Number = acx * apx + acy * apy + acz * apz;
							if (d1 <= 0 && d2 <= 0) {
								v = 0;
								w = 0;
								break;
							...
							var bpz:Number = pz - bz;
							
							var d3:Number = abx * bpx + aby * bpy + abz * bpz;
							var d4:Number = acx * bpx + acy * bpy + acz * bpz;
							...
							var cpx:Number = px - cx;
							var cpy:Number = py - cy;
							var cpz:Number = pz - cz;
							
							var d5:Number = abx * cpx + aby * cpy + abz * cpz;
							var d6:Number = acx * cpx + acy * cpy + acz * cpz;
						...
						p1x = u * v11x + v * v21x + w * v31x;
						p1y = u * v11y + v * v21y + w * v31y;
						p1z = u * v11z + v * v21z + w * v31z;
						p2x = u * v12x + v * v22x + w * v32x;
						p2y = u * v12y + v * v22y + w * v32y;
						p2z = u * v12z + v * v22z + w * v32z;
					...
					if ((v4x - v3x) * nx + (v4y - v3y) * ny + (v4z - v3z) * nz < 0.001 || separation >= 0) {
						if (hit) {
							sep.init(-nx, -ny, -nz);
							pos.init((p1x + p2x) * 0.5, (p1y + p2y) * 0.5, (p1z + p2z) * 0.5);
							//if (Math.random() > 0.5) pos.init(p1x, p1y, p1z);
							//else pos.init(p2x, p2y, p2z);
							dep.x = separation;
							return true;
						}
						return false; // no hit
		...
		override public function detectCollision(shape1:Shape, shape2:Shape, manifold:ContactManifold):void {
			...
			if (flip) {
				b = BoxShape(shape2);
				c = CylinderShape(shape1);
			} else {
				b = BoxShape(shape1);
				c = CylinderShape(shape2);
			}
			...
			if (state == 0) {
				...
				if (pd <= 0) {
					manifold.addPoint(v1x, v1y, v1z, nx, ny, nz, pd, true);
				}
				pd = nx * (v2x - ccx) + ny * (v2y - ccy) + nz * (v2z - ccz);
				if (pd <= 0) {
					manifold.addPoint(v2x, v2y, v2z, nx, ny, nz, pd, true);
				}
				pd = nx * (v3x - ccx) + ny * (v3y - ccy) + nz * (v3z - ccz);
				if (pd <= 0) {
					manifold.addPoint(v3x, v3y, v3z, nx, ny, nz, pd, true);
				}
				pd = nx * (v4x - ccx) + ny * (v4y - ccy) + nz * (v4z - ccz);
				if (pd <= 0) {
					manifold.addPoint(v4x, v4y, v4z, nx, ny, nz, pd, true);
				}
			} else {
				switch(state) {
				case 1:
				...
				if (dot < -0.96 || dot > 0.96) {
					...
					if (ed < 0) {
						manifold.addPoint(ex, ey, ez, nx, ny, nz, sd, false);
					}
				}
			}
		}
